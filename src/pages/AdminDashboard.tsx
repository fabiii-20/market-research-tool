        ...
        {activeTab === "reports" ? (
            ...
            <div className="px-4 sm:px-6 md:px-8 py-4 md:py-6 grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6">
              <div className="bg-[#242850] rounded-xl md:rounded-2xl p-4 md:p-6 flex items-center gap-4 md:gap-6">
                <div className="h-12 w-12 md:h-16 md:w-16 rounded-xl bg-indigo-600/20 grid place-items-center shrink-0">
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 md:h-8 md:w-8 text-indigo-400" viewBox="0 0 24 24" fill="currentColor">
                    <path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z" />
                  </svg>
                </div>
                <div>
                  <p className="text-gray-400 text-xs md:text-sm">Logged-In Users</p>
                  <p className="text-white text-2xl md:text-3xl font-bold">{totalUsers}</p>
                </div>
              </div>

              <div className="bg-[#242850] rounded-xl md:rounded-2xl p-4 md:p-6 flex items-center gap-4 md:gap-6">
                <div className="h-12 w-12 md:h-16 md:w-16 rounded-xl bg-emerald-600/20 grid place-items-center shrink-0">
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 md:h-8 md:w-8 text-emerald-400" viewBox="0 0 24 24" fill="currentColor">
                    <path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8l-6-6z" />
                  </svg>
                </div>
                <div>
                  <p className="text-gray-400 text-xs md:text-sm">Reports Generated</p>
                  <p className="text-white text-2xl md:text-3xl font-bold">{reportsGenerated}</p>
                </div>
              </div>
            </div>
            ...
            <div className="px-4 sm:px-6 md:px-8 py-3 md:py-4 bg-[#242850] mx-4 sm:mx-6 md:mx-8 rounded-xl md:rounded-2xl flex flex-col md:flex-row flex-wrap items-start md:items-center gap-2 md:gap-4">
              <div className="w-full md:w-auto flex flex-col sm:flex-row items-start sm:items-center gap-2 flex-1">
                <div className="flex items-center gap-2 w-full sm:w-auto">
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 md:h-5 md:w-5 text-gray-400 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                  </svg>
                  <span className="text-gray-400 text-xs md:text-sm shrink-0">From:</span>
                  <input
                    type="date"
                    value={fromDate}
                ...
                <div className="flex items-center gap-2 w-full sm:w-auto">
                  <span className="text-gray-400 text-xs md:text-sm shrink-0">To:</span>
                  <input
                    type="date"
                    value={toDate}
              ...
              <div className="bg-[#242850] rounded-xl md:rounded-2xl overflow-hidden">
                ...
                <div className="px-4 sm:px-6 py-3 md:py-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4 border-b border-gray-700/50">
                  ...
                  <div className="relative w-full sm:w-auto" ref={dropdownRef}>
                    ...
                    >
                      <span className="hidden sm:inline text-gray-400">Select:</span>
                      <span className="truncate flex-1 sm:flex-none">{selectedUser}</span>
                      <svg xmlns="http://www.w3.org/2000/svg" className={`h-3 w-3 sm:h-4 sm:w-4 transition-transform shrink-0 ${dropdownOpen ? "rotate-180" : ""}`} viewBox="0 0 20 20" fill="currentColor">
                      ...
                      <div className="absolute top-full mt-2 right-0 w-full sm:w-56 bg-[#1a1d3e] rounded-xl border border-gray-700 shadow-xl z-50 py-1 max-h-64 overflow-y-auto">
                        {userNames.map((user) => (
                          <button
                            ...
                            className={`w-full text-left px-3 sm:px-4 py-2 hover:bg-indigo-600/20 text-xs sm:text-sm ${selectedUser === user ? "bg-indigo-600/20 text-indigo-400" : "text-gray-300"}`}
                          >
                            {user}
                          </button>
                        ))}
                ...
                {reports.length === 0 ? (
                  <div className="p-6 sm:p-8 md:p-12 text-center text-gray-400">
                    {totalUsers === 0 ? (
                      <div>
                        <p className="text-sm sm:text-base md:text-lg font-semibold mb-2">No users have logged in yet.</p>
                        <p className="text-xs sm:text-sm">User reports will appear here once users log in through the user portal.</p>
                      </div>
                    ) : (
                      <div>
                        <p className="text-xs sm:text-sm md:text-base">No reports found for the selected filters.</p>
                        <p className="text-xs sm:text-sm mt-2">Try adjusting the date range or user.</p>
                      </div>
                    )}
                  </div>
                ) : (
                  <div className="overflow-x-auto">
                    <table className="w-full min-w-max">
                      <thead className="bg-[#1a1d3e] border-b border-gray-700/50">
                        <tr>
                          <th className="px-3 sm:px-6 py-2 sm:py-4 text-left text-xs font-medium text-gray-400 uppercase">DATE</th>
                          <th className="px-3 sm:px-6 py-2 sm:py-4 text-left text-xs font-medium text-gray-400 uppercase">KEYWORDS</th>
                          <th className="hidden sm:table-cell px-3 sm:px-6 py-2 sm:py-4 text-left text-xs font-medium text-gray-400 uppercase">USER</th>
                          <th className="px-3 sm:px-6 py-2 sm:py-4 text-left text-xs font-medium text-gray-400 uppercase">CATEGORIES</th>
                          <th className="px-3 sm:px-6 py-2 sm:py-4 text-left text-xs font-medium text-gray-400 uppercase">ACTIONS</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-gray-700/50">
                        {reports.map((report) => (
                          <tr key={report.id} className="hover:bg-gray-700/20">
                            <td className="px-3 sm:px-6 py-2 sm:py-4 whitespace-nowrap text-xs sm:text-sm text-white">{report.date}</td>
                            ...
                            <td className="hidden sm:table-cell px-3 sm:px-6 py-2 sm:py-4 whitespace-nowrap text-xs sm:text-sm text-white">{report.user}</td>
                            <td className="px-3 sm:px-6 py-2 sm:py-4 text-xs sm:text-sm text-gray-400 truncate max-w-[150px]">{report.categories?.join(", ") || "N/A"}</td>
          ...
          <div className="p-4 sm:p-6 md:p-8 flex-1 overflow-auto">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-4 mb-4 md:mb-6">
              <h2 className="text-base sm:text-lg font-semibold text-white">User Accounts</h2>
              <button
            ...
            <div className="bg-[#242850] rounded-xl md:rounded-2xl overflow-hidden">
              <div className="overflow-x-auto">
                <table className="w-full min-w-max">
                  <thead className="bg-[#1a1d3e] border-b border-gray-700/50">
                    <tr>
                      <th className="px-3 sm:px-6 py-2 sm:py-4 text-left text-xs font-medium text-gray-400 uppercase">USERNAME</th>
                      <th className="hidden md:table-cell px-3 sm:px-6 py-2 sm:py-4 text-left text-xs font-medium text-gray-400 uppercase">EMAIL</th>
                      <th className="px-3 sm:px-6 py-2 sm:py-4 text-left text-xs font-medium text-gray-400 uppercase">ADDED AT</th>
                      <th className="px-3 sm:px-6 py-2 sm:py-4 text-left text-xs font-medium text-gray-400 uppercase">STATUS</th>
                      <th className="hidden sm:table-cell px-3 sm:px-6 py-2 sm:py-4 text-left text-xs font-medium text-gray-400 uppercase">REPORTS</th>
                      <th className="px-3 sm:px-6 py-2 sm:py-4 text-left text-xs font-medium text-gray-400 uppercase">ACTIONS</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-700/50">
                    {users.map((user) => (
                      <tr key={user.id} className="hover:bg-gray-700/20">
                        <td className="px-3 sm:px-6 py-2 sm:py-4 whitespace-nowrap text-xs sm:text-sm text-white font-medium">{user.username}</td>
                        <td className="hidden md:table-cell px-3 sm:px-6 py-2 sm:py-4 whitespace-nowrap text-xs sm:text-sm text-gray-400 truncate max-w-[200px]">{user.email}</td>
                        ...
                        <td className="hidden sm:table-cell px-3 sm:px-6 py-2 sm:py-4 whitespace-nowrap text-xs sm:text-sm text-white">{user.reportsCount}</td>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-[#242850] rounded-xl sm:rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-4 sm:p-6 border-b border-gray-700/50 flex items-center justify-between">
              <h3 className="text-lg sm:text-xl font-bold text-white">Report Details</h3>
              <button
            ...
            <div className="p-4 sm:p-6 space-y-3 sm:space-y-4">
              <div>
                ...
                <p className="text-sm sm:text-base text-white font-medium">{viewReport.date}</p>
              </div>
              
              <div>
                <p className="text-xs sm:text-sm text-gray-400">User</p>
                <p className="text-sm sm:text-base text-white font-medium">{viewReport.user}</p>
              </div>
              
              <div>
                <p className="text-xs sm:text-sm text-gray-400 mb-2">Keywords</p>
                <div className="flex flex-wrap gap-2">
                  {viewReport.keywords.map((kw, i) => (
                    <span key={i} className="px-2 sm:px-3 py-1 bg-indigo-600/20 text-indigo-400 rounded-full text-xs sm:text-sm">
                      {kw}
                    </span>
                  ))}
                </div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {viewReport.categories?.map((cat, i) => (
                    <span key={i} className="px-2 sm:px-3 py-1 bg-emerald-600/20 text-emerald-400 rounded-full text-xs sm:text-sm">
                      {cat}
                    </span>
              ...
              <div className="pt-3 sm:pt-4 border-t border-gray-700/50">
                <p className="text-xs sm:text-sm text-gray-400 mb-2">Report Content</p>
