...
export default function AdminDashboard() {
  ...
  const fetchReports = async () => {
    ...
    try {
      const data = await reportAPI.getAdminSearchResults(
        selectedUser === "All Users" ? "all" : userIdMap[selectedUser] || selectedUser,
        reportsPage,
        10,
        fromDate,
        toDate
  ...
  fetchReports();
}, [selectedUser, reportsPage, fromDate, toDate, userIdMap]);
...
const handleViewReport = async (report: Report) => {
  ...
  return (
    <div className="min-h-screen bg-[#1a1d3e] flex flex-col md:flex-row">
      ...
      {/* Sidebar and Nav */}
      {/* (Keep your existing sidebar, header, navigation, logout, etc here as is) */}
          ...
          <>
            ...
            <div className="px-4 sm:px-6 md:px-8 py-3 md:py-4 bg-[#242850] mx-4 sm:mx-6 md:mx-8 rounded-xl md:rounded-2xl flex flex-col md:flex-row flex-wrap items-start md:items-center gap-2 md:gap-4">
              ...
              <div className="relative w-full sm:w-auto" ref={dropdownRef}>
                ...
                >
                  <span className="hidden sm:inline text-gray-400">Filter by:</span>
                    ...
                    {userNames.map((user) => (
                      <button
                        ...
                        className={`w-full text-left px-3 sm:px-4 py-2 hover:bg-indigo-600/20 text-xs sm:text-sm ${
                          selectedUser === user ? "bg-indigo-600/20 text-indigo-400" : "text-gray-300"
                        }`}
                      >
                        {user}
                      </button>
      ...
      {viewReport && pdfUrl && (
        ...
        >
          <div className="bg-[#2d2b3e] rounded-lg w-[90vw] max-w-4xl max-h-[90vh] overflow-auto relative shadow-lg">
            ...
            <iframe src={pdfUrl} title="Report PDF Viewer" className="w-full h-[80vh] rounded-b-lg" frameBorder="0" />
          </div>
        </div>
      )}
    </div>
